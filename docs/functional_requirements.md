...
In scope: the 188 MVP functional requirements covering the reporting platform (FR-1 … FR-83, FR-177), the billing, payment and subscription domain (FR-84 … FR-152), the cross-cutting obligations no single use case owns (FR-153 … FR-159), notifications (FR-160 … FR-173), and the advisor domain (FR-190 … FR-203, added 11 Sep 2026). The MVP packaging is Model 1 — freemium, direct-to-SME, VSME Basic Module — with three plans: Free, Standard and Enterprise (D-12).
...
- **Deliberate MVP exclusions with no requirement written against them:** Buyer and Licensee capability beyond the generic relationship model, and advisor entitlement sponsorship (D-16) — *advisor capability itself was promoted into MVP on 11 Sep 2026 as FR-190 … FR-203*; enterprise SSO; tenant MFA; XBRL export; the Comprehensive Module; usage-based pricing; reseller commissions; multi-currency price automation; direct debit and standing-order mandates; virtual cash register and eBon integration; double-entry accounting; and collection on behalf of customers. These are stated in section 4 of the design decisions document. Where a placeholder requirement already existed against one, it appears in section 8; the remainder carry no requirement at all.
...
| `UC-n` | Use case | `use_cases.md`, UC-01 … UC-211 |
...
|---|---|---|---|---|---|
| FR-1 | The system shall allow an account to be registered from an email address and password, creating an unverified account record and issuing a verification challenge, and shall make no application data reachable until verification completes. | MVP | Registration precedes any organization; nothing is reachable pre-verification (UC-01). **Note (20 Aug 2026, `architecture.md` OQ-51, OQ-53):** the policy the password is validated against is ≥ 8 and ≤ 128 characters requiring a lowercase letter, an uppercase letter, a digit and one further character; and a registration naming an address that already exists answers `409 Conflict` rather than a uniform response — NFR-64's uniform-response clause is scoped by its own citations to FR-4, FR-6 and FR-11 and does not reach this path. **Note (25 Aug 2026, `architecture.md` §12.5.6, task 26.2):** a registration presenting a live invitation token for that same address creates an **already-verified** account and issues no verification challenge — the invitation link is the same proof of mailbox control the challenge would seek, arriving by the same channel. The unverified record and the challenge remain this requirement's default and every other path. | UC-01 | Registration with email and password creates an unverified account record and issues a verification challenge; no application data is reachable while the account is unverified. |
| FR-2 | The system shall allow an account to be registered through a social identity provider — Google and Microsoft at MVP — requesting only identifier, email and display name scopes, and shall produce the same account record with the provider identity as its credential ~~and no password set~~ **— and shall hold the account in a setup state, able to do nothing but complete its setup, until its owner has set a password and supplied a given name and a family name and confirmed the interface language; an account abandoned in setup is deleted seven days after registration, except one moved into setup from active, which may hold organizations and is never deleted by that rule** (amended 14 Sep 2026, project owner; built by task 155; `architecture.md` §12.5.6's task-155 row). | MVP | D-6: social sign-in is MVP scope; another password is a real barrier at first use for the target user. **Amended 14 Sep 2026**: the barrier moves to the step after the provider rather than being removed, because an account holding nothing but a provider identity is stranded the day that provider is disabled (FR-82, UC-70) — found by task 67.11. | UC-02 | Provider registration requests only the three named scopes; the resulting account record is identical in kind to a password registration and holds the provider identity as credential; **it is not active until a password and both name parts are set, and until then every route that needs a session refuses it but its setup routes — sign-out and refresh need none and stay open, and refresh ends a setup past its deadline** (task 155). |
| FR-3 | The system shall verify control of a registered email address through a time-limited link, transitioning the account to active; shall expire unverified accounts after a defined window; shall treat verification as satisfied where a provider asserts an already-verified address; **and shall treat it as satisfied where the account is registered from a live organization invitation issued to that same address** (added 25 Aug 2026, `architecture.md` §12.5.6, task 26.2 — the invitation link is the same proof of mailbox control this requirement's own challenge seeks, delivered by the same channel). **For an account registered through a provider, a verified address satisfies verification but does not make the account active: it enters the setup state until its password and names are set (FR-2)**, and confirming by link opens its password step for 15 minutes (amended 14 Sep 2026, project owner; task 155). | MVP | **Note (20 Aug 2026, `architecture.md` OQ-52):** the "defined window" is **7 days**, after which the account record is deleted and the address is registrable again. It is a different object from the link's own lifetime, which is 24 h (§12.5.6). Enforced at the point of use from task 19; the sweep that reclaims the rows lands with the scheduler in Phase 6. **Note (20 Aug 2026, `architecture.md` OQ-55):** because the link's 24 h lifetime is shorter than the account's 7-day window, FR-3 is satisfiable only if a link can be reissued — `POST /api/v1/auth/verification-email` does so, answering uniformly whether or not the address is registered. | UC-03 | Following a valid, unexpired link transitions the account to active — **or, for an account registered through a provider and holding no password, into setup, with its password step open for 15 minutes (FR-2; task 155)**; an expired link does not; an unverified account is expired after the defined window; a provider-asserted verified address satisfies verification without a separate email. |
| FR-4 | The system shall authenticate by whichever credential the account holds and issue a session scoped to the user's organization memberships and roles, shall match a provider identity on its subject identifier rather than its email address, and shall rate-limit failed attempts and lock out after a threshold. | MVP | Credential-stuffing protection; matching on subject identifier keeps an account resolvable after the user changes their email at the provider (UC-04, UC-05). | UC-04, UC-05 | A password session and a provider session are identical in scope, and identical in lifetime wherever the person asked to be remembered — **amended 4 Sep 2026** (`architecture.md` OQ-35): S-01 now offers *Keep me signed in on this device*, so a password session takes one of §12.5.6's two lifetime pairs while a **provider** session is always issued the remembered pair, the provider buttons being plain anchors that carry no client JavaScript and so no toggled answer. Scope is unconditional; lifetime is the person's choice on every path that can express it — S-01's credential form, and since 14 Sep 2026 S-36's password step on S-02's path, which signs the person in (task 155). A provider identity whose email changed still resolves to the same account; failed attempts are rate-limited and the account locks out at the configured threshold. |
...
| FR ID | Requirement | Pri | Rationale / source | Source UC | Acceptance criteria |
|---|---|---|---|---|---|
| FR-9 | The system shall maintain a personal profile — **a given name and a family name**, contact email, **an optional job title and an optional phone number**, notification preferences — held independently of any organization the user belongs to, capturing the two name parts at registration and allowing them to be edited afterwards. **The display name is derived from them and is not stored** (`design_spec.md` UX-137). The per-category structure of those preferences is specified in FR-163. **Amended 12 Sep 2026** (project owner, with `design_spec.md` OQ-16's name half): this read *"display name"* as a single stored field, which no schema ever carried and which would have been a third value free to disagree with the parts it was built from. **Amended 23 Sep 2026** (project owner, task 52.3): the optional job title and phone number are the S-27 artboard's, added rather than left undrawn; the phone is used only for support to reach the person about their account. **The contact email is the sign-in address**, shown on the profile and not a second address — task 49.3's recorded assumption, made the decision, so no address the platform writes to is unconfirmed. | MVP | One account may hold roles in several organizations, so profile data is personal to the user (UC-13). Two parts rather than one because a monogram, a sort order and a formal-register salutation each need to know which part is which, and a single string cannot say. | UC-13, UC-168 | Profile values persist against the user and are unchanged by switching active organization; the derived display name matches UX-137 for both parts present, either absent and neither; notification preferences conform to FR-163. |
...
| FR-11 | The system shall accept an organization invitation bound to the invited email address, single-use and expiring, granting the assigned edit or view-only role on acceptance. | MVP | Binding to the invited address is what stops a social sign-in for a different address from consuming the invitation (UC-15). **Note (25 Aug 2026, `architecture.md` §12.5.6, task 26.2):** acceptance also sets the session's active organization to the one just joined, so S-03's exit holds for a caller who already belonged elsewhere; an address that already belongs to an **active** member consumes the invitation and leaves that member's role untouched, while a **removed** member's row is reactivated at the invited role (task 25.1). **Note (13 Sep 2026, `architecture.md` §12.5.6, task 142):** a valid, unexpired invitation may also be refused at acceptance — `409 entitlement-quota-exceeded` where the organization is already over its seat ceiling, which is reachable only once the ceiling was lowered after issue, and `503 seat-allowance-unavailable` where the ceiling cannot be read. Either refusal rolls the acceptance back whole, so the invitation stays pending and the same link works afterwards. The acceptance criterion's three conditions stay necessary; the seat ceiling adds a fourth. | UC-15 | An invitation is accepted only for the invited address, only once, and only before expiry; on acceptance the assigned role is granted scoped to that organization. |
...
| FR-15 | The system shall maintain the organization's legal form, registered name, registered address and contact details — **including the contact name and address printed on the report cover** — with every change attributed and timestamped. **Amended 29 Aug 2026 (project owner, task 30.3):** the report-cover contact was drawn on S-15 by `EasyESG Organization Admin.dc.html` and named by no requirement, which made it a field the screen would have invented. It is a *second* contact and not a rename of the existing one: the platform writes to `contactEmail` about the organization, while this is the person a reader of the published report contacts about its content, and the two are routinely different people. | MVP | These values propagate into every report the organization produces (UC-50); the cover contact is the one that propagates **visibly**, which is why it is stated here rather than left to the export to invent. | UC-50 | Each field is editable by the Organization Administrator, and each change records the acting user and a timestamp. A report cover contact name and address are held and are distinct from the platform contact details. |
| FR-16 | The system shall maintain entity identifiers with **IDNO as primary** and **LEI as an optional additional identifier**, validating format and checksum on entry. **Amended 18 Aug 2026** — see `architecture.md` §18 OQ-18: IDNO is universal, free and already the `billing` context's identifier, so it is the only candidate populated for every organization at signup, whereas LEI carries an annual fee and is held by very few Moldovan SMEs. LEI is retained as an optional field so B1 stays VSME-conformant for banks and EU buyers who require one. DUNS, EU ID and PermID are not modelled at MVP. | MVP | An identifier that fails validation downstream in EFRAG's own tooling is expensive to discover at filing time (UC-51). | UC-51 | An IDNO is recorded as the primary identifier and an LEI may be recorded alongside it as an optional additional identifier; an identifier failing format or checksum validation is rejected at entry. DUNS, EU ID and PermID are not offered. *(Corrected 28 Aug 2026 — this column still carried the pre-amendment scheme and contradicted its own requirement text; see `architecture.md` OQ-18, closed 18 Aug 2026.)* |
...
| FR ID | Requirement | Pri | Rationale / source | Source UC | Acceptance criteria |
|---|---|---|---|---|---|
| FR-24 | The system shall provide a guided stepped wizard covering all VSME Basic Module fields B1–B11, entered at the module where work last happened and, where nothing has been answered, at the first incomplete step, capturing structured and narrative content for a module together. **Amended 2 Sep 2026 (task 35.3, project owner):** the entry step was "the first incomplete step" alone; FR-39 restores a returning user's wizard position on any device, and one rule serves both — `architecture.md` §12.5.6 records it. | MVP | Narrative and quantitative content are captured together so a figure can be explained in context (UC-25). | UC-18, UC-19 … UC-29 (**UC-20** named explicitly — B2 is the one narrative module in the range; OQ-4) | All B1–B11 fields are reachable through the wizard; opening a report enters at the module with the most recent answer, or at the first incomplete step where nothing is answered; a module's structured and narrative content are captured in the same step. |
...
| FR-27 | The system shall pre-populate B1 from the entity master record while leaving the values editable in-report. **Amended 2 Sep 2026 (task 91.2, project owner):** *the entity master record* means the FR-18 point-in-time snapshot the period took at open, never the live record — an Administrator's later correction does not re-fill a filing already under way. A pre-populated value becomes the report's answer when the reporter leaves the field, and otherwise when the step it is shown on is **opened** — amended 3 Sep 2026 (task 36.2): UX-34's *step change* cannot mean departure for a value nobody typed, because the autosave queue is mirrored to durable storage in an effect and fires nothing on unmount, so a write enqueued as the step is left reaches neither. It is stored nowhere before the step is opened, and an unvisited step holds nothing; `architecture.md` §12.5.6 records both. | MVP | D-2: B1 is a disclosure, not master data; entity master data is Organization Administrator-owned, disclosure content is Contributor-owned. | UC-19 | B1 fields open pre-populated from the entity master record and remain editable in-report; editing them in-report does not alter the master record. |
...
| FR-29 | The system shall capture quantitative disclosures in the units the standard requires — MWh, tCO₂e, m³, kg or tonnes, hectares or km² — the hazardous and non-hazardous split, and the number of employees by contract type, gender and country of employment contract; and shall derive the intensity figures the standard specifies. **Amended 9 Sep 2026 (project owner, task 36.9), against EFRAG's published package.** The list named *headcount and FTE* among the units, and **neither is a unit the standard states**: task 91.4 measured the whole `measurementGuidance` role — 42 elements, of which **38 name a unit**, over seven codes (`MWh ha kg m3 sqkm t tCO2e`); the other four are the GHG intensity ratios, whose guidance is prose. B8's eight elements are in neither set, so its employee counts are figures the standard declares no unit for. That gap is `design_spec.md` **OQ-22**'s, which owns the 40-of-78 quantitative elements in the same position. **Headcount and FTE are real and are B1's, as the two admitted *bases* of a count** — `TypeOfNumberOfEmployees` draws on `HeadcountMember` and `Full-TimeEquivalentFTEMember`, both labelled in all three locales, and `EmployeeCountingMethodology` says whether the count is period-end or average; the Digital Template's B8 block declares its tables *"linked from B1"* for exactly that reason. **The first version of this amendment said no FTE existed anywhere and was wrong** (spec review, 9 Sep 2026): it searched element keys and element labels and never the enumeration domains or the member catalogues. What is true is narrower — there is no FTE *figure*; there is an FTE *basis*. | MVP | — | UC-21, UC-22, UC-24, UC-25, UC-26, UC-28 | Each named quantity is captured **in the unit the standard states for it, and with no unit where the standard states none** — never in a unit this platform chose; and the specified intensity figures are derived rather than typed. **The criterion's second clause is task 36.9's** (project owner, 9 Sep 2026): it read *"in the stated unit"* unconditionally while the requirement above now names quantities that have no stated unit, which made it unverifiable on the **40 of 78 quantitative elements** OQ-22 counts rather than false — OQ-22's own measured number, not a second derivation of it. Where several units are admitted the reporter picks one per element and the platform defaults to none — task 91.4's decision, and the reason a `kg`-shaped default is a 1 000× error rather than a convenience. |
...
| FR-31 | The system shall allow a disclosure section to be declared **omitted as classified or sensitive information**, state the omission in B1 as the standard requires, satisfy validation rather than suppress it, and carry the declaration into both export formats. **Amended 9 Sep 2026 (project owner, task 36.13), against the VSME standard text.** This requirement read *"declared **not material or not applicable** with a recorded rationale"*, and the standard permits neither: **¶19** allows omission on exactly one ground — *"when the provision of the disclosures in this Standard requires disclosing classified or sensitive information, the undertaking may omit such information"* — **¶21** says undertakings *shall report* on B1–B11, and VSME **has no materiality assessment at all**, double materiality having been judged too burdensome for SMEs. So a report using materiality as an omission ground would be non-conformant, and this requirement specified one. **The rationale clause goes with it**: **¶24(b)** asks the undertaking to *indicate the disclosure that has omitted*, never why. **"Not applicable" is real but is not this requirement's** — **¶13**'s *if applicable* principle is instruction-driven and *needs no explanation*, which is FR-28's conditional applicability, shipped as configuration in task 91.3. | MVP | A reader must see a stated omission instead of an unexplained gap (UC-30) — and must be able to trust that the ground is the one the standard permits. | UC-30 | A declared section is written to B1's `ListOfOmittedDisclosuresDeemedToBeClassifiedOrSensitiveInformation`, reports as satisfied rather than suppressed in validation, is discounted in the completion rollup, and appears in both the PDF and the Excel template export — natively in the latter, the element being one the template already carries. |
...
| FR-41 | The system shall roll validation state up per module and across the whole report, discounting sections declared omitted as classified or sensitive information (FR-31, as realigned 9 Sep 2026) so that a legitimate omission does not depress completion. | MVP | A legitimately excluded module must not depress the completion figure (UC-38). | UC-38 | Module-level and report-level states are derived from field states; a section declared omitted under FR-31 does not reduce the completion figure. |
...
| FR-57 | The system shall allow a user to be invited by email with an edit or view-only role, an unacted invitation to be resent, and an invitation to be revoked, with revocation invalidating the outstanding link immediately. | MVP | Inviting beyond the plan's seat entitlement triggers the quota path (UC-150); a capped number of users is included on Free because gating collaboration entirely would block the primary workflow (UC-60). **Note (13 Sep 2026, `architecture.md` §12.5.6, task 141):** both routes may also refuse with **429** when five invitations or resends have already gone to one address from this organization inside fifteen minutes. Recorded here because this requirement's routes are the ones that refuse, and because **no NFR stands behind it**: NFR-64 binds authentication paths, and §12.5.6 states in terms that issue and resend are not among them — so unlike the accept path's throttle, which FR-4 and NFR-64 already authorise, this one is an amplification control whose only home is here and in §12.5.6. It does not qualify the acceptance criterion above: a resend still delivers the same invitation. What it bounds is how often, to a third party who never asked. **Note (12 Sep 2026, task 142):** the seat allowance this row's rationale assumes is a **plan entitlement**, and AD-5's entitlement service does not exist until task 54. Until it does, the ceiling is a configuration artefact (AD-4) read behind one function, counted over active members ∪ pending invitations, **lapsed ones included** (corrected 13 Sep 2026 from *live pending*, §12.5.6's task-142 row) — so the quota path is real rather than assumed, and task 54.2 swaps the source without moving a call site. **Note (25 Aug 2026, `architecture.md` §12.5.6, task 26.1):** "a resend delivers the same invitation" is satisfied by the invitation *record* — a resend rotates the token and restarts the seven-day window, leaving exactly one live link per invitation; and an address already held by an active member or by a live pending invitation is refused rather than invited twice. | UC-60, UC-61 | An invitation is issued with the assigned role; a resend delivers the same invitation; after revocation the outstanding link is refused immediately. |
...
| FR-78 | The system shall issue scoped, time-limited support-access grants only on a request stating a reason and a ticket reference **and granted by an Organization Administrator of the named organization**, expiring automatically without administrator action. | MVP | D-5. **Amended 14 Sep 2026 (project owner, task 67.9): consent is the organization's.** A request is only a request — nobody at the platform can grant it on the organization's behalf — and while a grant lasts the organization's members see it (`design_spec.md` UX-124). A grant is read-only and lasts 60 minutes; a longer need is a new request with its own reason and its own grant; an unanswered request lapses after 24 hours; and any Organization Administrator of the organization, or any Platform Administrator, may end a grant early. `architecture.md` §12.5.6's task-67.9 row records the shape. | UC-85 | A grant requires a reason, a ticket reference and an Organization Administrator's grant; is limited to the named organization and to reading; and ceases to permit access at expiry with no action taken. A declined, lapsed or ended request permits nothing. |
...
| FR-80 | The system shall allow platform administrator accounts to be created, modified and deactivated with separable privilege levels, so that content, operations and support functions do not require one another's rights, **and shall allow an administrator to manage their own credentials — password, second factor and recovery codes — without another administrator's involvement**. | MVP | A translator does not require the privileges of a taxonomy migration operator (UC-87). **Amended 12 Sep 2026** (project owner, with task 144): this row covered *other people's* accounts only, and the realm shipped with no self-service at all — no password change, no re-enrolment, no recovery codes — leaving a sole operator who lost their second factor recoverable only through shell access to the provisioning CLI. That is a single point of failure no requirement had accepted in writing. | UC-87, UC-212 | An administrator can be granted content rights without operations or support rights, and each account can be deactivated; an administrator changes their own password and re-enrols a second factor without another administrator acting, and a recovery code is consumed exactly once. |
...
| FR-82 | The system shall allow social identity providers to be registered, enabled, disabled and have their credentials rotated without a redeploy, with disabling stopping new registrations while leaving existing accounts able to authenticate by another credential. | MVP | Keeps an expiring or leaked client secret from becoming a platform-wide outage; a provider can be withdrawn without stranding users (UC-70). **Note (14 Sep 2026, `architecture.md` §12.5.6's task-24 and task-67.11 rows):** the client secret rotates by an environment change and a restart until task 154 moves it into the secret manager; the client id, issuer, redirect addresses and enabled state rotate with no deployment through A-18. | UC-70 | Credentials are rotated with no deployment; after disabling a provider, new registrations and links through it are refused while existing accounts still authenticate by another credential. |
...
| FR-102 | On a quota-exceeded action the system shall block it, state which limit was reached and what the current plan allows, offer the upgrade path, and shall never discard reporting work in progress or prevent a started report from being finished and exported. | MVP | Reporting work already in progress is never lost to a quota block (UC-150). **Note (13 Sep 2026, `architecture.md` §12.5.6's task-142 row):** the interim seat ceiling's block states the limit reached, what the organization is allowed and current consumption, and **offers no upgrade path**, because no plan exists to upgrade to until tasks 53 and 63 — the project owner's S-16 decision of 12 Sep 2026. This row's upgrade-path clause is therefore **deferred for the seat ceiling, not met**: the block names a way out the reader can take instead (withdraw an invitation, or remove someone's access), and the path arrives with the plan catalogue. UX-50 does not itself permit the path's absence; S-16's entry attributed that to it until the parent-close review of task 142. | UC-150 | The blocked action reports the limit reached, the current plan allowance and the upgrade path; work in progress is retained; a started report can still be completed and exported. |
...
| FR-167 | The system shall cancel an outstanding notice and stop its repetition as soon as its condition clears — the disclosure supplied, the section declared omitted as classified or sensitive, the period locked — and shall deduplicate on category and subject so that a repeatedly evaluated condition produces one notice rather than one per evaluation. | MVP | — | UC-169, UC-170 | Supplying the disclosure, declaring the section omitted as classified or sensitive, or locking the period cancels the notice and stops repetition; repeated evaluation of one unchanged condition yields one notification, not one per run. |
| FR-173 | The system shall hold the notification category catalogue's behaviour as publishable configuration — default channels, transactional-or-optional classification, deadline lead times, repeat interval — so that tuning a notice needs no release, with its per-locale subject and body wording shipped in the release that raises it. | MVP | Tuning a notice is configuration, not a release (UC-176). **Amended 19 Aug 2026 (architecture.md OQ-43):** template wording moved into the message catalogues. A category cannot exist unless code calls `raise()` with its key — `RaiseNotificationCommand.categoryKey`, `apps/api/src/contracts/notification.port.ts` — so a genuinely new notice always arrives with a release regardless of where its words live, and "adding a notice needs no release" was only ever true of the catalogue row, not the trigger. What an operator actually tunes without a release is the behaviour, and that is unchanged. **Amended 21 Sep 2026 (project owner, task 49.3):** the classification of a **mandatory system category** — security, account, invoice delivery, payment failure, service restriction, and the identity notices registered under them — is declared in code, so no artefact can make one optional and an unreadable one still sends by email (`architecture.md` §12.5.6's task-49.3 row (1)); an optional category's classification stays configuration. | UC-175, UC-176 | Default channels, classification, lead times and repeat interval are configuration values edited and published with no deployment; subject and body wording resolves from the message catalogue by category key. |
...
| FR-169 | The system shall deliver by email through a provider reached behind the standard provider adapter, resolving language per recipient rather than per notification, and including a working one-click unsubscribe in every optional-category message. | MVP | NFR-11, FR-156: the provider sits behind the standard adapter; the target user is an SME owner who does not log in between reporting sessions (UC-173). | UC-173 | Two recipients with different email languages receive the same notification each in their own language — the email language chosen per person apart from the interface language, and starting as it (amended 23 Sep 2026, task 52.3); every optional-category email carries a working one-click unsubscribe; the provider is reached only through the adapter. |
...
| FR ID | Requirement | Pri | Rationale / source | Source UC | Acceptance criteria |
|---|---|---|---|---|---|
| FR-190 | The system shall allow an organization of the advisor type to be created from a verified account, shall automatically grant the creating user the Advisor Administrator role over it, and shall hold no reporting entities or reporting periods against it. | MVP | D-1 applied to the advisor type; FR-14 already admits the type without a schema change. An advisor firm reports on its clients' behalf and has nothing of its own to report. | UC-196 | An advisor organization is created from a verified account, the creating user holds Advisor Administrator over it, and no reporting entity or period can be created against it. |
| FR-191 | The system shall allow an Advisor Administrator to invite staff into the advisor organization and to set, per staff member, which clients on the roster that staff member may enter, enforcing the restriction at the point of entry rather than by omission from the interface. | MVP | Confidentiality between engagements is routinely contractual; a firm of fifteen does not give all fifteen access to all clients (UC-197). | UC-197 | A staff member outside a client's scope is refused entry to that client by the server, not merely shown a shorter list. |
| FR-192 | The system shall allow an advisor organization to request access to a client organization, creating a relationship in a pending state that confers no access to any of the client's data. | MVP | D-15: the advisor requests and the client grants. A pending request that could read anything would make the grant meaningless. | UC-198 | A pending relationship yields no read of any client record; the client's Organization Administrator is notified of the request. |
| FR-193 | The system shall present to the Advisor Administrator a roster of every client organization the firm has requested or been granted access to, with relationship state, granted entity scope, expiry date where set, and the client's plan. | MVP | The roster is the object the Advisor plan's client quota is counted against (FR-202). | UC-199 | Each roster entry shows state among pending, active, revoked and expired, together with scope and any expiry. |
| FR-194 | The system shall allow an Advisor Administrator to end a client relationship from the advisor side without requiring an action by the client, retaining the attribution of that firm's historical contributions in the client's change history. | MVP | Ending an engagement must not erase who entered which figure, for the same reason removing a member does not (FR-59, UC-63). | UC-200 | After the firm ends an engagement, its users can no longer enter the client, and prior field changes remain attributed to them. |
| FR-195 | The system shall allow an Organization Administrator to grant a pending advisor request over a selected set of reporting entities and an optional expiry date, defaulting the entity scope to none. | MVP | D-15. A default of none makes a careless grant an empty grant rather than a total one. | UC-201 | A grant made without selecting entities confers access to no entity; the granted scope and any expiry are recorded against the relationship. |
| FR-196 | The system shall allow an Organization Administrator to decline a pending advisor request with an optional reason, recording the declined request rather than deleting it. | MVP | Repeated unsolicited requests from one firm are only visible as a pattern if declines persist (UC-202). | UC-202 | A declined request remains visible to the organization with its date and any stated reason, and the outcome reaches the requesting firm. |
| FR-197 | The system shall allow an Organization Administrator to revoke an active advisor relationship, withdrawing access at the advisor's next request and terminating any advisor session then inside the organization at its next action. | MVP | Matches the immediacy already required of a role downgrade (FR-58, UC-62); a firm removed mid-engagement must lose access at once, not at next login. | UC-203 | After revocation, the next request from any user of that firm is refused and the reason is shown. |
| FR-198 | The system shall present to the Organization Administrator, in one place, both direct members and any advisor organizations holding access, with each advisor's granted scope, expiry and the individual advisor users able to enter. | MVP | "Who can see our ESG data" must have one complete answer; an access path absent from that view cannot be governed (UC-204). | UC-204 | The access view lists direct members and advisor organizations together, and every path by which a person can reach the organization's report data appears in it. |
| FR-199 | The system shall, when an advisor user enters a client organization, verify that the relationship is active, unexpired and within that staff member's scope, and shall set the tenant context to exactly one client organization for the resulting session. | MVP | D-15. Resolving to a single tenant is what leaves the row-level isolation predicate and its cross-tenant test unmodified; this is the extension point of FR-12's membership resolution. | UC-205 | An advisor session carries the tenant context of exactly one client organization; entry is refused where the relationship is inactive, expired or out of scope. |
| FR-200 | The system shall present to the Advisor Administrator a consolidated board showing, for every active client, each reporting entity and open period with due date, days remaining, completion percentage and validation state, assembled by a scoped query per client rather than by a query spanning organizations. | MVP | D-15. The board is the capability the Advisor plan is sold on, and the assembly rule is what keeps it from becoming an isolation exception (UC-206). | UC-206 | The board reflects every active client; no query backing it selects across organizations and none runs under a privilege that bypasses row-level isolation. |
| FR-201 | The system shall restrict the export of the consolidated board to status and deadline metadata, excluding disclosure content. | MVP | A single file must not become an unintended aggregation of several clients' report data (UC-207); exporting a client's report itself remains FR-48 and FR-49 inside that client's context. | UC-207 | The exported file contains no disclosure field value from any client. |
| FR-202 | The system shall support an Advisor plan whose entitlement keys — roster size, advisor staff seats, board access and board export — are evaluated against the advisor organization, and shall not permit any key of that plan to raise the entitlements of a client organization. | MVP | D-16. Sponsorship would require the entitlement service (FR-99) to accept a relationship as a further override source, and is deferred. | UC-208, UC-209 | A client organization on Free exhibits Free entitlements while an advisor on a paid plan is working inside it; the advisor firm receives one invoice for its own subscription. |
| FR-203 | The system shall end an advisor relationship automatically on its expiry date and shall notify the client's Organization Administrator and the advisor firm of a request and of each subsequent grant, decline, revocation or expiry, as a transactional notification category. | MVP | An expiry a person must remember is an expiry that will not happen; and a person may not opt out of being told that an outside firm asked to read their company's data (FR-166, UC-210, UC-211). | UC-210, UC-211 | The relationship ends on its expiry date with no human action and with advance notice; request and outcome notices are delivered and cannot be suppressed by a notification preference. |
...
| BR-APP-4 | Water relevance is sector-driven. **The clause *"and supports a documented immateriality determination"* is struck 9 Sep 2026** (task 36.13): VSME carries no materiality assessment, so there is nothing to document — a rule that is wrong about an undertaking is corrected as configuration (FR-72, UC-81), which `architecture.md` §12.5.6's task-36.7 row now records as the recourse. **The relevant sectors are NACE sections A–E, held as data and matched member-or-descendant against the B1 activity answer** (task 91.3; `architecture.md` §12.5.6). | FR-28, FR-31 |
...
| BR-DIS-2 | A section may be declared omitted only on the standard's own ground — classified or sensitive information (VSME ¶19) — is stated in B1 as ¶24(b) requires, satisfies validation, and carries into both export formats. **No rationale is recorded, because ¶24(b) asks which disclosure was omitted and never why**; this rule required one until 9 Sep 2026. | FR-31 |
...
| Entity identifier | IDNO (primary), LEI (optional additional), format and checksum validity — DUNS / EU ID / PermID not modelled at MVP (OQ-18, 18 Aug 2026; row corrected 28 Aug 2026) | FR-16 |
...
| Omitted-disclosure declaration | Section (VSME ¶24(b)) — **no rationale**, which the standard does not ask for | FR-31 |
