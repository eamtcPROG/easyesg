...
| FR-63 | The system shall allow an additional interface and export locale to be registered as a configuration and content task rather than a development task, with **Romanian (source), English and Russian live at MVP** — each separately authored, never machine-translated — and no architectural limit to three. | MVP | NFR-4: EFRAG's own template ships in eleven languages. | UC-73 | A third locale is registered and populated without a code change, and becomes selectable for interface and export. |
...
| FR-177 | FR-161 | Report authoring | Support the VSME Comprehensive Module [C1–C9] as an additive extension of Basic | P2 | MVP scope is the Basic Module. The internal schema already mirrors VSME element names and structure including C1–C9 (FR-155, NFR-2), so the module is additive rather than a rework. |
