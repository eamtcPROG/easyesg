...
3. **A known ID collision exists between the two primary sources and is not resolved by renumbering.** FR-160 … FR-173 are used twice: as MVP notification requirements in the FR register (the later document) and as deferred P2/P3 requirements in the FR Deferred Scope document (the earlier one). In this consolidated baseline **FR-160 … FR-173 mean the MVP notification requirements** in section 3.30 and 3.31. The deferred entries retain their source IDs verbatim in section 8, marked as colliding, and require reassignment above FR-173 before promotion into build scope. This is carried as an open question (OQ-1), not silently fixed.
...
| FR-16 | The system shall maintain entity identifiers with LEI as primary and DUNS, EU ID or PermID as fallback, validating format and checksum on entry. | MVP | An identifier that fails validation downstream in EFRAG's own tooling is expensive to discover at filing time (UC-51). | UC-51 | An LEI may be recorded as primary and a DUNS, EU ID or PermID as fallback; an identifier failing format or checksum validation is rejected at entry. |
...
### 3.5 Report authoring (FR-24 … FR-32)

| FR ID | Requirement | Pri | Rationale / source | Source UC | Acceptance criteria |
|---|---|---|---|---|---|
| FR-24 | The system shall provide a guided stepped wizard covering all VSME Basic Module fields B1–B11, entered at the first incomplete step, capturing structured and narrative content for a module together. | MVP | Narrative and quantitative content are captured together so a figure can be explained in context (UC-25). | UC-18, UC-19 … UC-29 | All B1–B11 fields are reachable through the wizard; opening a report enters at the first incomplete step; a module's structured and narrative content are captured in the same step. |
...
| FR ID | Requirement | Pri | Rationale / source | Source UC | Acceptance criteria |
|---|---|---|---|---|---|
| FR-61 | The system shall hold all field labels, help text and validation messages as versioned data editable through an administrative console, so that a wording correction reaches users without a release. | MVP | NFR-12: content-only changes must not require a redeploy, which is what makes a quarterly regulatory-watch cadence sustainable. | UC-71 | A label, help text or validation message is edited in the console and, once published, reaches users with no deployment. |
| FR-62 | The system shall publish a reviewed translation set as an explicit, versioned and reversible step taking effect across all tenants at once, so that a half-finished translation is never live. | MVP | Publishing is an explicit step rather than a side effect of editing (UC-72). | UC-72 | Edits are not live until published; publication applies across all tenants at once and can be reverted to the prior version. |
| FR-63 | The system shall allow an additional interface and export locale to be registered as a configuration and content task rather than a development task, with Romanian and English live at MVP and no architectural limit to two. | MVP | NFR-4: EFRAG's own template ships in eleven languages. | UC-73 | A third locale is registered and populated without a code change, and becomes selectable for interface and export. |
| FR-64 | The system shall log every runtime fallback to the default locale as a reviewable queue of untranslated content keys. | MVP | Turns localization gaps from user-reported complaints into a maintained work queue (UC-74). | UC-14, UC-74 | Rendering a string with no translation in the active locale adds its key to the reviewable queue. |
...
| FR-173 | The system shall hold the notification category catalogue and its templates as localized, publishable configuration — default channels, transactional-or-optional classification, deadline lead times, repeat interval — on the same mechanism as FR-61 and FR-62, so that adding a notice or changing its wording needs no release. | MVP | Adding a notice or changing its wording is configuration, not a release (UC-176). | UC-175, UC-176 | A category is added and a template edited per locale through the FR-61/FR-62 mechanism and published with no deployment; default channels, classification, lead times and repeat interval are configuration values. |
...
| Entity | Attributes named by requirements | Requirements |
|---|---|---|
| Content string | Key, locale, value, version | FR-61, FR-62 |
| Translation set publication | Version, published state, reversibility | FR-62 |
| Locale registration | Interface and export availability | FR-63 |
| Fallback log entry | Content key, locale | FR-64, FR-10 |
...
| Notification category catalogue | Default channels, transactional-or-optional classification, deadline lead times, repeat interval, templates per locale | FR-173 |
...
| EFRAG XBRL converter (MIT-licensed, self-hostable) | Convert stored data to Inline XBRL, XBRL-JSON, XBRL-CSV | FR-160 (deferred set) | P2 |
| Energy providers and accounting software | Pluggable ingestion connectors | FR-171 (deferred set) | P3 |
| External document-risk service (e.g. IFC MALENA) | Advisory-only consistency warnings on narrative disclosures | FR-173 (deferred set) | P3 |
...
**Identifier warning.** The IDs below are reproduced verbatim from "ESG Platform Functional Requirements — Deferred Scope, Coverage and Traceability (MVP)". Fourteen of them — FR-160 … FR-173 — collide with the MVP notification requirements in sections 3.30 and 3.31, which were authored later in the same sequence. Per section 2.2 rule 3, **in this baseline FR-160 … FR-173 mean the MVP notification requirements**; the deferred entries carrying those numbers are marked `⚠ collides` and require reassignment above FR-173 before promotion into build scope. Nothing here is renumbered by this document. See OQ-1.

| FR ID (verbatim) | Collision | Domain | Requirement | Phase | Deferral rationale |
|---|---|---|---|---|---|
| FR-160 | ⚠ collides with MVP FR-160 | Export | Convert stored data to Inline XBRL, XBRL-JSON and XBRL-CSV through EFRAG's self-hosted open-source converter | P2 | XBRL export is a Phase 2 exclusion by prior decision; the MVP export target is the Excel Digital Template (FR-50), and the converter is placed behind an internal interface so the addition is additive (FR-156, NFR-11). |
| FR-161 | ⚠ collides with MVP FR-161 | Report authoring | Support the VSME Comprehensive Module [C1–C9] as an additive extension of Basic | P2 | MVP scope is the Basic Module. The internal schema already mirrors VSME element names and structure including C1–C9 (FR-155, NFR-2), so the module is additive rather than a rework. |
| FR-162 | ⚠ collides with MVP FR-162 | Reporting oversight | Provide completion-status dashboards with deadline reminders | P2 | Deferred by prior decision. The MVP substitutes are the organization-wide status overview (FR-23) and the deadline notice (FR-165). |
| FR-163 | ⚠ collides with MVP FR-163 | Comparatives | Provide a standalone year-over-year analytics view across periods and entities | P2 | D-3: comparative storage and inline display are MVP (FR-45, FR-46, FR-47); only the standalone analytics view is deferred. |
| FR-164 | ⚠ collides with MVP FR-164 | Identity | Support enterprise SSO — federated SAML/OIDC against a customer's own directory, with domain claiming, just-in-time provisioning and directory-driven deprovisioning — distinct from the social sign-in in MVP scope | P2 | D-6: social sign-in is MVP, enterprise SSO is not. It becomes relevant when Advisor and Corporate Buyer organizations arrive; the provider-agnostic identity model keeps it a provider registration rather than a rework. |
| FR-165 | ⚠ collides with MVP FR-165 | Identity | Support multi-factor authentication for ordinary tenant users, already required at MVP for the Platform Administrator (FR-75) | P2 | Deferred for tenant users by prior decision; mandatory now only for the role with cross-organization visibility (FR-75). |
| FR-166 | ⚠ collides with MVP FR-166 | Advisor | Manage a portfolio of client organizations from one login, completing or reviewing a report on a client's behalf | P2/P3 | Phase 2/3 actor capability. The typed organization relationship model is built at MVP so the actor type is added without a schema change (FR-14, NFR-9). Sequencing is demand-driven on the MVP success metrics (FR-83). |
| FR-167 | ⚠ collides with MVP FR-167 | Corporate buyer | Invite and monitor supplier organizations, view aggregated and benchmarked dashboards, and request a supplier's VSME data with consent | P2/P3 | As FR-166: Phase 2/3 actor capability anticipated by FR-14 / NFR-9, sequencing demand-driven. |
| FR-168 | ⚠ collides with MVP FR-168 | Licensee | Administer a white-labelled instance — branding, domain, language pack — and the sub-organizations under it | P2/P3 | Phase 2/3 actor capability; this is where the re-scoped Moldova/MDED licensing scenario sits. Anticipated by FR-14 / NFR-9 and by locale registration (FR-63, NFR-4). |
| FR-169 | ⚠ collides with MVP FR-169 | Billing | Support usage-based and metered pricing units, for which FR-105 already emits the events and NFR-10 already requires multi-unit entitlement | P2 | Usage-based pricing is not offered at MVP by prior decision, but the event stream (FR-105) and multi-unit entitlement (NFR-10) exist so activation is additive. |
| FR-170 | ⚠ collides with MVP FR-170 | Billing | Support reseller and partner commission handling for the Model 3 and Model 5 monetization scenarios | P2/P3 | Belongs with the Model 3 and Model 5 monetization scenarios, which are not activated at MVP. |
| FR-171 | ⚠ collides with MVP FR-171 | Data ingestion | Support pluggable connectors for energy-provider and accounting-software data ingestion | P3 | Phase 3 exclusion. MVP captures consumption by manual entry in invoice units (FR-33); connectors sit behind the internal-interface rule (FR-156). |
| FR-172 | ⚠ collides with MVP FR-172 | Report authoring | Provide AI-assisted narrative drafting for qualitative fields [B2/C2] with mandatory human review before save, authorship remaining with the company | P3 | Phase 3 exclusion. NFR-6 constrains any AI feature: no training on submitted customer data and a stated deletion/retention policy. |
| FR-173 | ⚠ collides with MVP FR-173 | Report authoring | Optionally call an external document-risk-flagging service (e.g. IFC MALENA) to surface advisory-only consistency warnings on narrative disclosures | P3 | Phase 3 exclusion; advisory-only by design. Vendor sits behind the internal-interface rule (FR-156, NFR-11). |
| FR-174 | — | Public disclosure | Provide an opt-in, searchable public disclosure portal, field-structured to align with anticipated ESAP requirements | P3 | Phase 3 exclusion; opt-in by design and structured now only in anticipation of ESAP. |
| FR-175 | — | Submission | Provide an ESAP-compatible submission bridge | Roadmap | Not committed. Recorded as a roadmap placeholder; needs a concrete problem statement before being scoped. |
...
| FR-10 (old) | LEI primary, DUNS/EU ID/PermID fallback | FR-16 |
| FR-11 (old) | More than one user per org viewing/editing a shared report | FR-25, FR-57, FR-58 |
| FR-12 (old) | XBRL conversion via the EFRAG converter (P2) | FR-160 (deferred set — see section 8 collision warning) |
| FR-13 (old) | Comprehensive Module [C1–C9] (P2) | FR-161 (deferred set) |
| FR-14 (old) | Completion dashboards and deadline reminders (P2) | FR-162 (deferred set) |
...
| FR-16 (old) | Energy-provider and accounting connectors (P3) | FR-171 (deferred set) |
| FR-17 (old) | AI-assisted narrative drafting (P3) | FR-172 (deferred set) |
| FR-18 (old) | External document-risk flagging (P3) | FR-173 (deferred set) |
...
|---|---|---|---|
| OQ-1 | **How are the colliding deferred FR IDs reassigned?** FR-160 … FR-173 denote MVP notification requirements in the register and deferred P2/P3 items in the Deferred Scope document. The deferred set needs IDs above FR-173, and the reassignment must be recorded as a mapping so existing citations (including legacy FR-12 … FR-18 in section 9.5) still resolve. Also logged in `non_functional_requirements.md` OQ-1. | C-1, sections 2.2 and 8 | Two requirements answer to one ID. A backlog item or test case citing FR-163 is ambiguous between notification preferences and the year-over-year analytics view. |
| OQ-2 | **Is the Billing Operator (BO) actor added to the actor register in `actors.md`?** `use_cases.md` introduces BO and records it as a recommended addition; 27 use cases and much of FR-84 … FR-152 depend on it. SYS is likewise absent from the actor register. Also logged in `use_cases.md` OQ-1 and `actors.md` OQ-1. | Use case register actor table; C-7 | The actor register and the requirements disagree on who exists, and the privilege separation between PA and BO (FR-80, FR-139) has no authority in the actors document. |
| OQ-3 | **What are the non-functional bounds on notifications?** Delivery latency, retry bounds for FR-171, email deliverability targets for FR-169, and the retention period for FR-170 delivery records are unspecified in every source. Four bounding requirements are now proposed as `NFR-106` … `NFR-109` in `architecture.md` §17.3, pending ratification into `non_functional_requirements.md`. Also logged in `use_cases.md` OQ-5, `non_functional_requirements.md` OQ-13, `architecture.md` OQ-2 and `design_spec.md` OQ-11. | Register closing note; G-5 | FR-169, FR-170 and FR-171 are functionally testable but have no acceptance threshold. "Bounded schedule" cannot be verified against a number. |
| OQ-4 | **Should UC-20 be named explicitly against FR-24?** UC-20 (B2 narrative) is traceable only through the `UC-19 … UC-29` range, unlike every other Basic Module use case. | G-3 | The trace to the report's principal narrative module depends on a reader expanding a range. |
