...
| FR ID | Requirement | Pri | Rationale / source | Source UC | Acceptance criteria |
|---|---|---|---|---|---|
| FR-9 | The system shall maintain a personal profile — **a given name and a family name**, contact email, notification preferences — held independently of any organization the user belongs to, capturing the two name parts at registration and allowing them to be edited afterwards. **The display name is derived from them and is not stored** (`design_spec.md` UX-137). The per-category structure of those preferences is specified in FR-163. **Amended 12 Sep 2026** (project owner, with `design_spec.md` OQ-16's name half): this read *"display name"* as a single stored field, which no schema ever carried and which would have been a third value free to disagree with the parts it was built from. | MVP | One account may hold roles in several organizations, so profile data is personal to the user (UC-13). Two parts rather than one because a monogram, a sort order and a formal-register salutation each need to know which part is which, and a single string cannot say. | UC-13, UC-168 | Profile values persist against the user and are unchanged by switching active organization; the derived display name matches UX-137 for both parts present, either absent and neither; notification preferences conform to FR-163. |
