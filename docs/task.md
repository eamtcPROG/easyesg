...
| 27 | Opt-in TOTP and password change | api+web | TOTP enrolment, challenge on sign-in, recovery codes — API and screens together. **Plus FR-7, change own password** (architecture.md OQ-56, 21 Aug 2026): it needs an authenticated caller and a settings surface, and this is the security-settings slice | 2FA e2e: enrol, challenge, recover; password change with optional termination of other sessions | TODO |
