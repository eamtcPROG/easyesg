...
| **Romanian and English** only | Revised Scope and Decision | **Superseded.** Three live locales: Romanian (source), English, Russian. Note the standing caveat: EFRAG publishes no Russian template, so Russian VSME labels are platform-authored and carry no official standing; an export intended for a bank or EU buyer should be produced in RO or EN |
...
| OQ-5 | **Narrowed 18 Aug 2026 — one half answered, the other is an action not a question.** Verified: EFRAG now ships the Digital Template **and the taxonomy labels** in multiple EU languages (the May → October 2025 releases added them, against the earlier English-only template), with Romanian-language VSME materials in circulation. EFRAG does not publish the label-language list, so confirming Romanian specifically requires downloading the current template — an action for the `config/efrag` seeding task, not an open question. | **The decision-relevant half is settled: Russian is not an EU language and is definitively not in EFRAG's official set.** So Russian VSME labels are platform-authored with no EFRAG standing regardless of how the Romanian question lands, which confirms the export caveat (`design_spec.md` UX-47, UX-98) and NFR-24's RO/EN-only scope as correct. If Romanian labels are official, platform-authored label content shrinks to Russian alone |
