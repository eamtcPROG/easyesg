...
| The lock does not reach a `DELETE` of a disclosure value — task 34.1 | **An open hole, recorded rather than closed** (1 Sep 2026). `refuse_locked_write` on `core.report_disclosure_value` covers `INSERT` and `UPDATE` and not `DELETE`, following task 31.2's decision and for its reason: a `BEFORE DELETE` trigger fires during a referential cascade, and the chain is organization → period → report → values, so covering `DELETE` would make a locked report's organization undeletable. **FR-22 as amended says the lock "refuses every write", so this contradicts it rather than narrowing it**, and nothing else closes it: `esg_app` holds `DELETE`, and there is no use case above this store — a first draft of the migration comment claimed the use case refused it, which was false and is the more dangerous half, since it reads as covered. **Owner: task 36**, which writes the first caller. Closing it needs a mechanism the cascade tolerates — a statement-level guard, or withholding `DELETE` from `esg_app` and removing a repeating-group row by state instead, which costs the natural-key row a way to disappear. Task 31.2's own §12.5.6 row still describes a `BEFORE UPDATE OR DELETE` trigger and is stale in the same direction; corrected there. | FR-22, UC-58, P-4, task 36 |
