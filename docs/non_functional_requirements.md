...
6. **A known ID collision affects `FR-n` citations inside section 6 and is not resolved by renumbering here.** See section 9, C-1.
...
| NFR-47 | The system shall keep marginal infrastructure cost per active free-tier organization below the stated monthly ceiling (D-12) | — (unquantified in source; the ceiling is referred to but not stated — see OQ-3) | **A** — monthly per-organization cost attribution derived from metering events | MVP | D-12; FR-105 (metering stream) |

### 4.3 Availability and continuity (NFR-48 … NFR-53)

| NFR ID | Requirement | Metric / target | Verification | Pri | Related FR / UC / D |
|---|---|---|---|---|---|
| NFR-48 | The system shall achieve ≥ 99.5% monthly availability outside the filing window and ≥ 99.9% within it, with announced maintenance capped at 4 hours per month outside the window and none within it | ≥ 99.5% monthly outside the filing window; ≥ 99.9% within it; announced maintenance ≤ 4 h per month outside the window; zero within it | **A** — production availability SLI, monthly review | MVP | — |
...
### 4.5 Security (NFR-13 ▪, NFR-60 … NFR-70)
...
Covers GDPR and Moldovan data-protection law. The Moldovan instrument is not cited by article number in any source; the register states the obligation as "Moldovan data-protection law" and this document does not supply a citation it does not have (see OQ-5).

| NFR ID | Requirement | Metric / target | Verification | Pri | Related FR / UC / D |
|---|---|---|---|---|---|
| NFR-5 ▪ | The system shall be hosted in the EU/EEA and operated in compliance with GDPR and Moldovan data-protection law | Hosting within the EU/EEA; compliance with GDPR and Moldovan data-protection law; records of processing maintained. No numeric target applies — this is a binary compliance obligation | **I** — annual compliance review; records of processing maintained | MVP | — (no FR written against it — see 7.3) |
...
| NFR-27 | The system shall hold all customer data at rest — primary store, replicas, backups, generated exports and logs — in EU/EEA regions | 100% of customer data at rest in EU/EEA regions, across primary store, replicas, backups, generated exports and logs | **A** region assertion in deployment checks · **I** quarterly sub-processor register review | MVP | — |
...
| NFR ID | Requirement | Metric / target | Verification | Pri | Related FR / UC / D |
|---|---|---|---|---|---|
| NFR-75 | The system shall meet WCAG 2.1 AA on all tenant-facing screens and shall produce tagged PDF exports with a correct reading order | WCAG 2.1 AA on 100% of tenant-facing screens; PDF exports tagged with a correct reading order | **A** automated accessibility pass in CI · **T** manual keyboard and screen-reader audit of the wizard | MVP | — (amendment proposed downstream — see C-3) |
...
| NFR-23 | The system shall hold Romanian as the source locale and English as a separately authored locale, not a machine translation (UC-72) | Romanian as source locale; English separately authored; zero machine-translated live content | **I** — editorial sign-off recorded per translation publication | MVP | UC-72 (amendment proposed downstream — see C-3) |
...
| NFR-25 | The system shall permit an additional locale to be added through content and configuration alone, with no code change, schema change or redeploy | A locale added with zero code changes, zero schema changes and zero redeploys | **D** — a third locale added in staging | MVP | Verifies NFR-4 |
...
| NFR-20 | The system shall produce EFRAG Excel exports that open without a repair prompt in Microsoft 365, current Excel and LibreOffice Calc, with named ranges, dropdowns and formulas intact (FR-50) | Zero repair prompts in Microsoft 365, current Excel and LibreOffice Calc; named ranges, dropdowns and formulas intact; per template version | **T** — automated open-and-diff in both applications per template version | MVP | FR-50 (verification amendment proposed downstream — see C-3) |
...
| NFR-82 | The system shall produce record-intended PDF exports conforming to PDF/A-2b (FR-49, FR-53) | PDF/A-2b conformance for 100% of record-intended PDF exports | **T** — conformance validation in the export regression suite | MVP | FR-49, FR-53 (amendment proposed downstream — see C-3) |
...
| NFR ID | Requirement | Metric / target | Verification | Pri | Related FR / UC / D |
|---|---|---|---|---|---|
| NFR-12 ▪ | The system shall support a quarterly regulatory-watch update cadence with no redeploy for content-only change (FR-74) | Quarterly cadence sustainable; zero redeploys for content-only change | Decomposed into NFR-85 and NFR-86 | MVP | FR-74 |
| NFR-85 | The system shall publish a content-only change — labels, help text, validation messages, thresholds, factor sets — to production within one working day of approval, reversible in a single step (FR-61, FR-62, FR-71 … FR-73) | ≤ 1 working day from approval to production; revert achievable in a single step | **D** — timed publish-and-revert rehearsal against production configuration | MVP | FR-61, FR-62, FR-71 … FR-73 |
...
| NFR-91 | The system shall raise operational alerts for unreconciled settlements beyond tolerance, e-Factura rejections, dunning backlog, metering-event lag, migration-run failures and locale-fallback spikes (FR-152, FR-64, UC-130) | Alerts present for all six named conditions; each exercised at least annually. Tolerance and lag thresholds: — (unquantified in source; held as operational configuration) | **I** alert inventory reviewed per release · **T** each alert exercised at least annually | MVP | FR-152, FR-64, UC-130 |
...
|---|---|---|---|---|---|
| NFR-94 | Security | The system shall validate federated SSO assertions for signature, audience and replay, and shall support domain claiming and directory-driven deprovisioning within a stated latency (D-6, FR-164) | Signature, audience and replay validated on every assertion; domain claiming and directory-driven deprovisioning supported. Latency: — (unquantified in source) | P2 | D-6, FR-164 † |
| NFR-95 | Security | The system shall support multi-factor authentication for ordinary tenant users, already mandatory for the Platform Administrator at MVP (NFR-65, FR-165) | MFA available to ordinary tenant users | P2 | NFR-65, FR-165 † |
| NFR-96 | Architecture & modularity | The system shall support per-tenant data residency and branding isolation for white-label Licensee instances (NFR-9, FR-168) | Per-tenant data residency and branding isolation | P2/P3 | NFR-9, FR-168 † |
...
| NFR-98 | Standards conformance & data fidelity | The system shall generate iXBRL, XBRL-JSON and XBRL-CSV instances validating against the EFRAG taxonomy under the official validator with zero errors (NFR-18, FR-160) | Zero validator errors against the EFRAG taxonomy, all three instance formats | P2 | NFR-18, FR-160 † |
| NFR-99 | Integrity & reliability | The system shall reconcile every usage-priced invoice line to its individual metered events, re-derivable on dispute (NFR-57, FR-169) | 100% of usage-priced invoice lines re-derivable from individual metered events | P2 | NFR-57, FR-169 † |
...
| NFR-101 | Data protection & privacy | The system shall publish no benchmarking figure derived from a cohort below the stated minimum size (FR-167) | Zero published figures below the minimum cohort size. Minimum size: — (unquantified in source) | P3 | FR-167 † |
| NFR-102 | Data protection & privacy | The system shall enforce human review before save of generated narrative content and shall record model and prompt provenance (NFR-6, FR-172) | Human review enforced before save on 100% of generated narrative content; model and prompt provenance recorded | P3 | NFR-6, FR-172 † |
| NFR-103 | Integrity & reliability | The system shall handle connector rate limits, backfill and source restatement of historic data without corrupting a reported figure (NFR-19, FR-171) | Zero corruption of a reported figure under connector rate limiting, backfill or source restatement | P3 | NFR-19, FR-171 † |
...
**† These `FR-n` citations resolve against the *deferred* functional register, not the MVP one.** The IDs collide: FR-160 … FR-173 denote MVP notification requirements in the current FR register and deferred P2/P3 capabilities in the FR Deferred Scope companion. Each `†` citation above means the deferred reading — FR-160 is XBRL conversion, FR-164 enterprise SSO, FR-165 tenant MFA, FR-167 corporate-buyer benchmarking, FR-168 white-label administration, FR-169 usage-based pricing, FR-171 ingestion connectors, FR-172 AI-assisted narrative drafting. FR-174 (public disclosure portal) and FR-144 do not collide. Nothing is renumbered here; see section 9, C-1, and OQ-1.
...
| # | Conflict | Resolution |
|---|---|---|
| **C-1** | **`FR-n` ID collision affecting section 6.** The deferred NFR companion cites FR-160, FR-164, FR-165, FR-167, FR-168, FR-169, FR-171, FR-172 and FR-174 with the meanings they carry in the **FR Deferred Scope** document (XBRL, enterprise SSO, tenant MFA, buyer benchmarking, white-label, usage pricing, connectors, AI narrative, public portal). The current MVP functional register has since assigned FR-160 … FR-173 to **notification requirements**. Both readings are live. | Both preserved verbatim. Section 6.2 marks every affected citation `†` and states the deferred reading explicitly, so no citation is ambiguous to a reader of this document. Neither set is renumbered here: reassigning the deferred FR IDs is the functional register owner's decision, is already carried as OQ-1 in the consolidated functional baseline, and is repeated here as OQ-1. FR-144 and FR-174 do not collide. |
...
| **C-3** | **A downstream document proposes eight amendments to this register that have not been ratified into it.** "ESG Platform System Architecture (MVP)" §15.1 records, as review-agreed: NFR-82 raised from PDF/A-2b to **PDF/A-2a + PDF/UA-1**; NFR-75 raised from WCAG 2.1 AA to **WCAG 2.2 AA**; NFR-48 **scoped to application availability**, excluding single-host infrastructure loss; NFR-47's ceiling **set at €0.50 per active free-tier organization per month**; **opt-in TOTP for tenant users at MVP**, against the design decisions' deferral and the deferred NFR-95; **Russian added as a third live locale**, against NFR-23's RO/EN (also logged in `design_spec.md` OQ-1 (locale count) and `actors.md` OQ-8 (tenant MFA)); the filing window **corrected to April–May** on the authority of Article 33(3) of Law 287/2017; and NFR-20's verification **split** into a LibreOffice Calc CI gate plus a manual Microsoft 365 release-checklist item, since server-side Office automation on Linux is unsupported. | **The register text in section 4 stands unamended.** These are proposals recorded by the architecture document itself as "needing to be reflected in the source registers; until they are, this section is the record". Consolidating them here would import numeric targets that no NFR source states — precisely what this baseline must not do. Each is flagged inline in section 4 against the affected requirement (`amendment proposed downstream`), and ratification is carried as OQ-3. **One exception: the filing-window correction has since been ratified** — it amends the §3 scale envelope rather than any NFR statement, imports no numeric target into the register, and closes OQ-2. NFR-47's ceiling is the remaining amendment that closes an open question in this document, so it is the one most worth ratifying next. |
| **C-4** | **A second, worse collision inside the proposed amendments.** The architecture document authors four notification qualities as **NFR-94 … NFR-97** (dispatch latency p95 ≤ 60 s; exponential retry bounded at 24 hours with hard-bounce suppression; ≥ 99% accepted transactional-mail delivery; delivery records retained for the life of the organization plus one year). Those four IDs are **already occupied** by the deferred register: NFR-94 federated SSO assertion validation, NFR-95 tenant MFA, NFR-96 per-tenant residency, NFR-97 Enterprise availability commitment. | The deferred register wins: **NFR-94 … NFR-105 mean what section 6.2 says they mean.** The four proposals have since been **renumbered to `NFR-106` … `NFR-109` in `architecture.md` §17.3**, above the deferred register, so the collision is now **closed** rather than open. They are recorded in OQ-13 as proposals at those IDs, pending ratification into this register, and are not entered into any register in this document. Their substance is real and closes the section 7.4 gap; only their original numbering was wrong. |
...
Fifteen items. OQ-4, OQ-6, OQ-7, OQ-8, OQ-9, OQ-10 and OQ-15 exist because a source states an obligation without a threshold; no number has been supplied for any of them here.

| # | Question | Origin | Consequence if unresolved |
|---|---|---|---|
| **OQ-1** | **How are the colliding deferred FR IDs reassigned?** FR-160 … FR-173 denote MVP notification requirements in the FR register and deferred P2/P3 capabilities in the FR Deferred Scope document. Nine citations in section 6.2 depend on the deferred reading. Also logged in `functional_requirements.md` OQ-1. | C-1; carried from the consolidated functional baseline's OQ-1 | Nine deferred NFRs cite an ambiguous functional ID. NFR-98 "cites FR-160" resolves either to XBRL conversion (correct) or to the notification record model (wrong). |
...
| **OQ-3** | **What is NFR-47's monthly cost ceiling per active free-tier organization?** The requirement obliges the system to stay below "the stated monthly ceiling"; no source NFR states it. A downstream document proposes €0.50 per organization per month, functioning as an alarm threshold rather than an operating constraint. | NFR-47; C-3 | The only cost requirement in the register is unverifiable. A freemium model with an unbounded marginal cost per free organization is an architectural failure, which is why the requirement exists at all. |
| **OQ-4** | **What are NFR-64's numeric bounds?** The lockout threshold, token entropy and token lifetime are all stated qualitatively — "rate-limit and lock out", "single-use, high-entropy, time-limited". | NFR-64 | The abuse-path test suite has no pass condition. "High-entropy" cannot be reviewed against a number, and FR-4's "locked out after a threshold" has no threshold either. |
| **OQ-5** | **Which Moldovan data-protection instrument does NFR-5 refer to?** Every source says "Moldovan data-protection law" or "Moldovan data-protection compliance" without naming an act or article. The task brief for this consolidation refers to Moldova Law 133, which no source in this project cites. | NFR-5, NFR-27 | The annual compliance review has no named instrument to review against, and the divergences between GDPR and the Moldovan act — which drive retention and cross-border transfer decisions — cannot be identified. The reference must be confirmed and recorded in the register rather than assumed here. |
...
| **OQ-9** | **What are NFR-84's transactional-mail delivery rate and NFR-71's e-Factura reconciliation tolerance?** Both requirements say "the stated" without a source stating it. A downstream document proposes ≥ 99% accepted delivery for mail. | NFR-84, NFR-71; C-3 | Deliverability monitoring has no alarm point. More seriously, NFR-71's daily reconciliation is required to show "zero unacknowledged invoices beyond tolerance" — with no tolerance, the check cannot distinguish a timing lag from a compliance failure under a mandate that takes effect 1 October 2026. |
| **OQ-10** | **What is NFR-88's per-component test coverage floor?** Five components are named — calculator, validation engine, entitlement service, invoice numbering, VAT calculation — with no percentage. | NFR-88 | The CI coverage gate has no threshold, and the requirement's real content (report per component, not as a project-wide average) is enforceable while its target is not. |
...
| **OQ-13** | **What are the non-functional bounds on notifications, and under which IDs?** FR-160 … FR-173 have no NFR counterpart: dispatch latency, retry bounds, deliverability and delivery-record retention are all unspecified. A downstream document proposes four requirements — dispatch p95 ≤ 60 s; exponential retry bounded at 24 h with suppression on first hard bounce; ≥ 99% accepted transactional-mail delivery, SPF/DKIM/DMARC-aligned; delivery records retained for the life of the organization plus one year — but originally numbered them NFR-94 … NFR-97, which the deferred register already occupies. **The four proposals now carry `NFR-106` … `NFR-109` in `architecture.md` §17.3** — above the deferred register, so the collision is closed — and they remain pending ratification into this register. Also logged in `use_cases.md` OQ-5, `functional_requirements.md` OQ-3, `architecture.md` OQ-2 and `design_spec.md` OQ-11. | §7.4; C-4; the FR register's own closing note; carried from the functional baseline's OQ-3 | FR-170's delivery records are described in the source as the evidence that a required update was actually requested — a compliance artefact with no retention period and no acceptance threshold. FR-171's "bounded schedule" cannot be verified against a number. Ratifying the proposals under the wrong IDs would put two meanings on four identifiers. |
| **OQ-14** | **Does NFR-36's verification cover NFR-72's assertion?** NFR-72 requires retrieval independent of the application, the database and the hosting provider; NFR-36, its named verification, tests independence from the application only. | C-8 | A six-year statutory archive is asserted to survive loss of the hosting provider without any evidence that it does — the failure mode a statutory archive exists for. |
