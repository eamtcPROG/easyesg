...
| NFR-69 | The system shall hold secrets in a managed secret manager, scoped per environment and access-logged, with none present in source, container images or configuration artefacts | Zero secrets in source, container images or configuration artefacts; all secrets in a managed secret manager, scoped per environment and access-logged | **A** — secret scanning in CI and on the image registry | MVP | **Note (14 Sep 2026):** the social providers' client secrets are environment variables until task 154 provisions the secret manager — a recorded deviation, `architecture.md` §12.5.6's task-24 configuration row — and A-18 reports only whether each is held and where it is set |
...
| NFR-27 | The system shall hold all customer data at rest in EU/EEA regions — **wherever it rests and whoever holds it, sub-processors included** — at providers not subject to third-country access law, **relying on no adequacy decision and no standard contractual clauses** to place it there. The primary store, replicas, backups, generated exports and logs enumerate *where* data rests; they are not a closed list of *whose* systems are in scope. **Amended 12 Sep 2026 (OQ-17), one narrow and reversible carve-out: the transactional mail channel.** The outbound email provider may be a non-EU/EEA one while the platform is pre-production, and is `smtp.gmail.com` meanwhile. Nothing else moves: the primary store, replicas, backups, generated exports and logs remain bound by the sentence above, as does every other sub-processor. The carve-out is **not** a judgement that the position was too strict — it is an acknowledgement that the mail channel is the one sub-processor the product cannot run without and the one whose replacement is an environment value rather than a migration | 100% of customer data at rest in EU/EEA regions across primary store, replicas, backups, generated exports and logs, and across every sub-processor holding customer data; zero reliance on an adequacy decision or on SCCs for data at rest. The mail channel is excepted per OQ-17 and asserted separately: `EMAIL_HOST` must name a member of the permitted set, which is the deployment check task 51.1 builds | **A** region assertion in deployment checks · **I** quarterly sub-processor register review | MVP | NFR-5. **Amended 25 Aug 2026 (OQ-16)** — the enumeration is illustrative rather than exhaustive, and the no-transfer-mechanism position is stated here rather than left to `architecture.md` §12.5 to act on. **Amended 12 Sep 2026 (OQ-17)** — the mail carve-out above, taken by the project owner with the conflict stated: Gmail is Google LLC, subject to FISA 702 and the CLOUD Act, and verification and reset mail carries addresses and links |
...
| NFR-23 | The system shall hold Romanian as the source locale, with English **and Russian** as separately authored locales, neither a machine translation (UC-72) | Romanian as source locale; English and Russian each separately authored; zero machine-translated live content across three live locales | **I** — editorial sign-off recorded per translation publication, per locale | MVP | UC-72. **Ratified 18 Aug 2026** (architecture.md §17.1). Architecturally free (NFR-4, NFR-25) and it replaces the staging-locale rehearsal. Real costs: a third translation set reviewed on every content publish, and **no official EFRAG source for Russian *or Romanian* VSME labels** — **amended 31 Aug 2026** (project owner) from *"NFR-24 applies to RO/EN only"*, which was wrong about Romanian: opening the `2026-05-01` package for task 33.2 showed its `ro` label linkbase is a stub carrying zero labels, one of eleven such files among 23 shipped, and the Digital Template's own language selection offers the same twelve that do. So **NFR-24 applies to English only** at this version, and Romanian and Russian labels are both platform-authored with no EFRAG standing (design_spec.md UX-47, UX-98). The editorial cost is therefore two authored label sets, not one — and the source locale is one of them. Closes design_spec.md OQ-1 and actors.md OQ-9. **Scope amended 19 Aug 2026 (architecture.md OQ-42), with the `apps/admin` scaffold:** the three live locales bind the **tenant-facing** interface, the exports and email. The *administrative console*'s own chrome is **Romanian only** at MVP and its URLs carry no locale segment — its users are a small internal population of Platform Administrators and Billing Operators, and separately authoring a third console chrome buys nothing while costing the same editorial review as tenant content. This narrows which surfaces ship three catalogues and nothing else: every console string remains a message key rather than a literal, and FR-63's "no architectural limit" is preserved — adding a console locale is authoring one more catalogue file. **Further amended 19 Aug 2026 (architecture.md OQ-43):** the console's keys resolve from a committed catalogue rather than the configuration store, so NFR-85 no longer binds here; a console wording change ships with a release. The Romanian-only decision and the absent locale segment are unaffected — both rest on locale count, not on where the catalogue lives. The three-locale registry is still read by the console, because A-03 is the screen on which an operator authors all three and registers a fourth |
| NFR-24 | The system shall use the official EFRAG translation of a VSME label wherever one is published (FR-51) | Official EFRAG translation used for 100% of VSME labels where one is published | **I** — label diff against the official template per version rollout | MVP | FR-51. **Which languages that reaches is a property of each release, not a constant** (noted 31 Aug 2026, task 33.2): at `2026-05-01` EFRAG ships label linkbases for 23 languages and only twelve carry labels, so this binds **English alone** of the three live locales — Romanian's file is a stub. The verification above is what re-checks it per rollout, and it must compare *content*, not the presence of a file: reading the directory listing as a translation set is the error this note corrects |
...
| **OQ-17** | **Raised and closed 12 Sep 2026 — the transactional mail channel is excepted from NFR-27 while the platform is pre-production, and the provider is Gmail meanwhile.** Authority: project owner, with the conflict stated before the decision rather than discovered after it. Gmail is Google LLC: subject to FISA 702 and the CLOUD Act, which is the *third-country access law* NFR-27 names, and verification, reset and invitation mail carries an address and a single-use link. The decision is therefore a knowing exception, not a reading of the requirement that makes one unnecessary — and it is narrow: only the mail channel moves, and the primary store, replicas, backups, exports, logs and every other sub-processor stay bound. | Raised while building `task.md` 51.1, whose row already required a deployment check asserting `EMAIL_HOST` against a permitted set *because* a config-driven provider is as easy to point somewhere non-compliant as somewhere compliant. That check is built; Gmail is added to the set rather than the check being skipped, so the exception is enumerated and visible instead of implicit. **What reverses it:** one environment value. The adapter speaks SMTP and no vendor type crosses the port, so an EU provider is a host, a port and a credential — not a class. **What else it costs, recorded because it is not obvious:** free Gmail caps at **500 messages per day** and Workspace at 2,000, against a filing season concentrated in April–May and a ≤2,000-organization envelope; NFR-108's ≥99% accepted delivery cannot be met through a hard daily cap, so the carve-out has an operational ceiling as well as a legal one. **What must change before production:** `EMAIL_HOST` points at a permitted EU provider, this row closes as superseded, and NFR-27's carve-out sentence is deleted rather than left as a dormant permission. | Project owner — decided |
