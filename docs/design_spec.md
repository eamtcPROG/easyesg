...
**A session the api has ended counts as none, wherever this tier learns it** *(added 16 Sep 2026 with task 160)*. The browser's cookie outlives a session ended elsewhere — a reset, *sign out other devices*, an operator's action — until its access token falls due, and a reader in that window is holding nothing the api will honour. So the branch above answers *sign in* for it and the gate serves the form rather than turning the reader away; S-35, which says sign-in succeeded, is never the answer; and any request refused that way — a screen's own read, or since task 161 a save the reader has just made — sends the reader to sign in with the address kept, under the first clause, rather than drawing *try again later* or a refusal above the form *(extended 21 Sep 2026, project owner, task 161: reads and writes alike)*.

**It binds the administrative realm too, and each realm's destination is its own** *(added 16 Sep 2026 with task 113)*. The rule was written while reading `apps/web`, so *the branch above* names §4.3's; the console has no membership branch and its destination is **A-01's exit** — a same-app `?redirect=`, and the operator's console home otherwise. The failure is the same one and slightly worse: served the form again, an operator holding a live console session re-runs the two-step handshake and rotates the sealed cookie underneath a session that was working. **A-01 has met the rule since task 113**, in both directions; `_realm`'s closed-by-default guard has met the first half since task 23. **Two things about the console's half are not the tenant app's**, because the realm cookie is the api's by `architecture.md` OQ-17 and unreadable here: the gate is a session probe rather than a cookie read, and a probe that cannot answer renders the form — the console's only way in may not depend on the api being reachable, and this gate spares an operator a mistake rather than refusing an attacker.

**A control that *offers* a session-issuing address is not a violation of this rule, and its wording may become one.** The public tier's *Sign in* and *Create an account* are session-blind by §5.1b's own requirement — *"they carry no session: nothing on them may read or imply an active organization"* — so a signed-in reader who presses one is answered by the branch, which is the right destination and needs no variant. What is not covered by that is a control offered as a **remedy** to a reader the screen already knows is signed in: S-03's unusable-invitation exit and the acceptance-problem callout both read *"Go to sign in"* to someone who has one, so the words described something that cannot happen, which is `CLAUDE.md`'s user-facing-text rule rather than this one. **Since task 114** each offers the remedy that fits the reader it has — S-03's entry says which — and the destination for a signed-in reader is this rule's branch, resolved for the session they hold.
...
- **Controls and actions:** invite by email with an edit or view-only role; resend an invitation; revoke an invitation; change a role; remove a member; promote a member to Organization Administrator; send a manual reminder to a user about an outstanding report (UC-175). **Amended 22 Sep 2026 (project owner, task 50.3):** the reminder is a panel below the list, the invite panel's anatomy — the person (any active member but the sender), one of the organization's open reports, an optional note of up to 500 characters, and *send* — with its outcome reported beside the form (`architecture.md` §12.5.6, task-50.3 rows (1), (4), (5)). Its states: ready; **nothing to remind about** — no open report, or no one but the sender — saying which; **partial** — the members or the reports could not be read, said in the panel alone while the list above stands; pending; sent; refused.
...
- **Exits:** S-17 or S-18 from the entitlement gate; S-05; S-06 from the reminder panel when no report is open (task 50.3).
...
- **Controls and actions:** open a notification, which records it read; mark read; mark all as read; dismiss; route to preferences. **Amended 22 Sep 2026 (project owner, task 50.2):** opening records the notice read, and *mark all as read* — drawn on both the page and the panel — marks every notice the unread count counts (`architecture.md` §12.5.6, task-50.2 row (2)). The list filters on read state — *Unread* and *All* — and the category filter waits until two categories can reach the centre (row (4)), which task 51.2's row carries (row (7)). **Amended 22 Sep 2026 (project owner, task 50.2's parent close):** the list is ordered newest first or oldest first, an address like the tabs — the Index archetype's sort (§4.6), which the artboard does not draw — while the panel is always newest first (row (6)).
- **States:** empty — first use (teaching empty state); empty — filtered; loading — initial; loading — refresh; error — recoverable; **error — permission** (added 22 Sep 2026 with task 50.2's parent close, S-06's addition of 5 Sep applied where it also holds: the centre is open to every member, so a request whose organization did not resolve arrives here rather than at a role refusal. It was implemented before it was listed, which is the omission UX-90 calls a defect).
...
- **Entry points:** the post-sign-in branch (§4.3), by password or by provider, when the membership
  ...
  **Not when the read fails because the session has ended** (task 160): the api refusing the session
  is not *could not determine*, and this screen's first sentence — that sign-in succeeded — would be
  false. That reader is sent to sign in, and arriving here with such a session redirects there.
...
- **Archetype:** Focus.
- **Entry points:** direct arrival at the administrative host; A-20's success, which lands here with the new operator's account created and a notice saying so (task 67.4). **An arrival carrying a live console session is answered by this screen's own exit rather than by the form** (UX-136, task 113) — by address as well as by control, so a bookmark and a typed address are gated like a link. It follows that an operator who accepts an invitation for a **second** account while signed in as their first is returned to their first account's home, and signs out before using the new one; the arrival notice buys no exemption, since it is in the address and anyone could put it there.
- **Layout and regions:** single column, centred, one primary action. No further per-screen layout is specified in the source.
...
**Amended 12 Sep 2026 with `architecture.md` AD-15.** Where a surface is accelerated by the push accelerator, the accelerator is how it updates **sooner** without raising a poll frequency — it is this rule's remedy, not an exception to it. Intervals do not change because a socket exists, and `architecture.md` OQ-36 sets them: unread count 60 s, S-16's access list 30 s, order state 3 s within its bounded window, export job state 5 s. Every poll stops while the tab is hidden, which is the control that makes the budget hold rather than the intervals themselves.

**UX-138** *(Added 12 Sep 2026 with AD-15.)* A surface accelerated by the push accelerator shall be **indistinguishable from the same surface on its poll, except that it updates sooner**. No control, no copy and no screen state shall depend on the connection being live: there is no *connected*, *disconnected* or *reconnecting* affordance, and §8.1's state set gains none.
