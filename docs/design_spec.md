...
| OQ-15 | **The icon set is unchosen.** UX-87 requires "one coherent set at 20 px and 24 px on a 24 px box, each with an accessible name, none the sole carrier of state or action". OQ-2 closes the visual identity layer and names this as the one remaining placeholder | **Open.** Prototypes stand in text glyphs (`✓ ! ≠ × — ⊘ 0`) for the eight disclosure states. UX-102 already forbids information carried by shape alone, so the set is constrained but not chosen. Blocks Phase 1 (component library) |
