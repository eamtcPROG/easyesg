...
**The first clause says *that* the gate answers and *what it must preserve*; it takes no position on how, and deliberately does not cite UX-38 for the whole of it.** UX-38 governs a narrower case — *session **expiry***, with UC-07's precondition *"a session has expired through inactivity or timeout"* — where there is preserved context and queued work to return to, and it requires re-authentication *"inline over the preserved context, never as a redirect to a blank sign-in screen"*. This clause is wider: a visitor who has never signed in reaches it too, and has no context for anything to be inline over. So the two compose rather than one containing the other. **Where both apply, UX-38 governs and is unmet today** — the session tier redirects with `?return=`, which is what UX-38's second sentence rules out. **Task 92 owns that for S-07**, the wizard, which is where the queued work UX-38 exists to protect actually is; the same gap on a cold `/home` or `/account/credentials` is unowned and unscheduled, and saying so is the point of this sentence. Nothing task 92 ships changes this rule.
