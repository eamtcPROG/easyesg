...
| `notification-category.reporting.manual_reminder.json` | `notification_category` | UC-175's manual reminder about an open report (task 50.3) — **the first `optional` category and the first travelling `in_app`**, and `in_app` alone: an optional category's email ships only once task 52.2's one-click unsubscribe has landed (`architecture.md` §12.5.6's task-49.3 row (7)), and 52.2's row publishes this artefact again with `email` (the task-50.3 row (2)) |
