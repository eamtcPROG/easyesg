...
no presentation role, or an explicit axis that resolves no members each fail the run and name what
they found. Each of those three assertions exists because it caught a real defect on first use —
`architecture.md` §7.3 records what the third one found.
