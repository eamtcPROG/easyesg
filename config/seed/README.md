...
Each file is one artefact: `<kind>.<scope>.json`, holding the payload. The kind is the first segment,
dashes folded to underscores; the scope is everything after it, taken as written — **dots and
underscores included since task 49.1**, so a notification category's scope is its key exactly
(`notification-category.identity.password_reset.json` → `notification_category`, `identity.password_reset`). The loader publishes a file
...
| `identity-provider.google.json` | `identity_provider` | FR-82: a social provider's behaviour — enabled state, client id, issuer, scopes, redirect allowlist — is store data so it can be withdrawn or rotated without a redeploy (A-18's screen since task 67.11). Ships **disabled with an empty client id**: enabling is a deployment's decision, made on A-18 by saving the provider's client id and enabling it once the server holds its secret. The client secret is deliberately NOT here — it is an environment variable until OpenBao exists (§12.5.6's task-24 configuration row) |
...
| `disclosure-applicability.vsme.json` | `disclosure_applicability` | **FR-28's four conditional-applicability rules** (task 91.3), which FR-72 makes configuration in terms: thresholds move with the standard and with Moldova's transposing legislation, so the ≥50 turnover threshold, the ≥150 gender pay gap threshold, the site-driven biodiversity condition and the water sector list are published rather than released. **Scope is the standard**, on the row above's reasoning — the numbers are VSME's own, and what legislation moves in time is carried by the store's own effective dating. A rule says what makes a field *apply*, so an element no rule names always applies and a taxonomy version that adds a hundred elements needs no line here. The water rule names NACE **sections** and is matched member-or-descendant, so a stored class (`nace:NACE_C1071`) satisfies `nace:NACE_C`; `applicability-rules.service.spec.ts` holds every element key and member in it against **both** registered versions, because unlike the `vsme-*` files this one is hand-written |
| `disclosure-axis-shape.vsme.json` | `disclosure_axis_shape` | **Which explicit axes are breakdowns rather than classifications** (task 36.4). An explicit axis says an element is reported along a fixed domain and says nothing about *how* it is asked: B3's energy is answered for every member — total, renewable, non-renewable — while B4's 94 pollutants and B7's 973 waste categories are domains a reporter selects from, and one row per member there is not a screen. **Nothing in EFRAG's package distinguishes them**, so it cannot be derived: member count is an invented threshold and reading the default member's English words is not a rule. Data rather than code because the judgement is per axis and reviewable, and a new module's axis needs no redeploy (AD-4, DR-3). Seeded with **one breakdown and three classifications**: `BreakdownOfEnergyConsumptionAxis` is the breakdown (task 36.4), and `TypeOfPollutantAxis` (36.5), `TypeOfWasteAxis` (36.8) and `CountryOfEmploymentContractAxis` (36.9) are classifications. `ReportingScopesAxis` is task 79.x's. **The two lists are read together and degrade independently** — an unreadable `classification` list leaves the breakdowns intact and vice versa, since a module that loses one should not lose the other. Reading it **fails closed** — an unreadable payload yields no breakdowns and a loud log line, because the opposite fallback renders B7 as 973 fields. |
| `disclosure-derivation.vsme.json` | `disclosure_derivation` | **Which Basic-module figures the platform computes, and from what** (task 36.10). EFRAG's Digital Template derives two — B8's `EmployeeTurnoverRate` and B9's `RateOfRecordableWorkRelatedAccidentsInTheReportingPeriod` — and **the taxonomy carries the results without the inputs**, so this is the only place the derivation is written down. Each entry names the derived element, a **formula kind** (`turnover_rate`, `accident_rate`) and its operands, each bound either to a `disclosure` (a reportable element) or to an `input` (a value with no element, stored in `core.report_derivation_input` — §7.3). **The offers are here and the arithmetic is not**: EFRAG prints 2 000 hours for a full-time working year and says in the cell that it varies by country and collective agreement, which is a threshold AD-4 makes configuration; a formula belongs to a template version, which DR-4 pins per report, so it ships as code in `models/derivation.model.ts`. Reading it **fails closed** and a malformed derivation is dropped **whole** — the alternative is a rate computed from a half-read operand map and filed with `origin = 'calculated'`, which says the platform stands behind it. Scope is the standard |
| `notification-category.identity.email_verification.json` | `notification_category` | **A notification category's behaviour** (task 49.1, FR-173): which channels it travels on and whether a recipient may turn it off (FR-163). **One artefact per category, scoped by its key**, so A-17 (task 67.10) publishes and reverts one category without touching the others (NFR-85). **A category is registered by the task that first raises it** — FR-173's own reasoning, since a category cannot exist unless code raises it — so these four are the notices that send today, each `email` and `transactional`, the owner's decision of 21 Sep 2026 (§12.5.6's task-49.1 row). **No wording**: the key is also the message-catalogue path the subject and body resolve by (OQ-43). **No cadence**: lead times and a repeat interval arrive with the first category that has them (51.2). Reading it **fails closed** — an absent or malformed payload answers no behaviour and logs the revision to replace — and `notification-category-catalog.service.spec.ts` holds the shipped set to exactly `NOTIFICATION_CATEGORY`'s members |
| `notification-category.identity.password_reset.json` | `notification_category` | As above: FR-6's reset link, and task 155's first-password wording of it, which is one trigger and so one category |
| `notification-category.identity.invitation.json` | `notification_category` | As above: FR-11's invitation to an organization |
| `notification-category.platform.admin_invitation.json` | `notification_category` | As above: an operator's invitation to the console (task 67.4) |
| `notification-category.reporting.manual_reminder.json` | `notification_category` | UC-175's manual reminder about an open report (task 50.3) — **the first `optional` category and the first travelling `in_app`**. `in_app` alone until task 52.2.2, whose one-click unsubscribe is what an optional category's email waited for (`architecture.md` §12.5.6's task-49.3 row (7)); since then `in_app` and `email`, and each of its emails carries FR-169's footer and RFC 8058's headers (the task-52.2 row) |
| `seat-allowance.global.json` | `seat_allowance` | **The interim seat ceiling** (task 142, `architecture.md` §12.5.6): how many people an organization may have — active members plus pending invitations, lapsed ones included. **Configuration and not an entitlement** (DR-1), read through `SEAT_ALLOWANCE` so task 54.2 swaps the source for the entitlement service and **deletes this file**; extended instead, it would become a second, permanent source of truth about seats. **10, and not the Free plan's cap** — `use_cases.md` OQ-8 leaves that a commercial decision and it stays open. One `global` scope, no per-organization override. Reading it **fails closed**: an absent or malformed payload (anything but a whole number of at least one) refuses invitations and acceptances with a loud log line naming the revision, and `seat-allowance.service.spec.ts` holds the shipped file readable |
| `disclosure-template-default.vsme.json` | `disclosure_template_default` | **The answers EFRAG's Digital Template ships a field already holding** (task 36.11). Measured across all four disclosure sheets' answer column, the Basic module ships exactly one: B10's minimum-wage affirmation at `D147`, printed `YES`. **Distinct from the derivation artefact's `default`**, which is an *offer* a reporter may accept without it becoming their answer (B9's 2 000 hours); a value here is committed through task 91.2's default path and becomes a stored answer with `origin = 'reported'`. Data rather than a branch because it is EFRAG's content and changes when the template does (AD-4, DR-3); the entity snapshot outranks it where both name an element. Reading it **fails closed** and a malformed entry is dropped whole — a half-read payload would commit an answer nobody made. C6/C7 carry thirteen more for task 79.x. Scope is the standard |
...
Taxonomy mappings (tasks 67.6, 67.7), unit lists (91.4), factor sets (37), validation rules (40),
