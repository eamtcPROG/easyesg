...
| Workspace | State |
| --- | --- |
| `apps/api` | Module tree (35 registered, empty), response envelope, problem+json filter, port surface, OpenAPI emission, the migration runner — §7.1's five schemas + `btree_gist`, plus `core.organization` — fifteen §7 invariants each proving its own rule bites, the tenant transaction (`TenantTransactionGuard` binding `app.current_org` transaction-locally, commit in an interceptor, rollback in the filter), **RLS enabled and forced on the tenant root** proven as both `esg_app` and the owning role, the **append-only substrate** — `audit.enforce_append_only(regclass)` plus a partitioned `audit.system_audit_log` — and **per-field audit capture** by trigger onto `core.field_change`, unbypassable and unforgeable. No `AuthGuard`, no controller but health |
