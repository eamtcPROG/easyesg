...
| `packages/contracts` | The wire contract. `openapi/v1.json` carries the seven `/auth` routes (tasks 19 and 21); since task 20 `src/generated/v1.ts` (openapi-typescript) plus hand-curated aliases and the RFC 9457 `ProblemDocument` are the exported surface, regenerated and diffed by the same `openapi:check` gate as the spec |
