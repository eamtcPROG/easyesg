...
*these 44 files*: where things are, what has already bitten someone, and what finishing looks like.

## Current state

44 components in nine folders, 20 spec files, `src/styles/tokens.css` at 198 lines. Not every
...
| `navigation/` | 5 | GlobalBar, AccountMenu, WorkspaceNav, LanguageSwitcher, Pagination |
...
```
src/
├─ index.ts        The barrel — 42 exports. `@easyesg/ui`
...
- **The four vocabularies live in directive-free sibling modules and are exported from the barrel
  *directly*.** `button-vocabulary.ts`, `data-table-vocabulary.ts`,
  `language-switcher-vocabulary.ts`, `version-pin-indicator-vocabulary.ts` — none carries
...
- **24 of the 44 modules carry `'use client'`, and each one needs a reason.** A hook, a browser API
