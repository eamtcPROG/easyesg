...
export class NotificationRaisedHandler implements JobHandler {
  ...
  async handle(payload: Record<string, unknown>, context: JobContext): Promise<void> {
    ...
    const raisedAtMicros = occurredAtMicrosOf(payload, NOTIFICATION_RAISED);
    const { unresolved } = await this.deliver.execute({
      notice,
      organizationId,
      raisedAtMicros,
 ...
 * refused here rather than reaching `app.current_org`, where the policies' cast would fail the job less legibly. The
 * row's time, beside it too, is `occurredAtMicrosOf`'s to read.
 */
