...
describe('PreviewUnsubscribe (task 52.2.2)', () => {
  ...
  it('offers the switch while the category still reaches the person by email', async () => {
    ...
    expect(await new PreviewUnsubscribe(plainTokens, optional, store, ANA).execute({ token })).toEqual({
      standing: 'available',
  ...
  it('says it is already switched off', async () => {
    ...
    expect((await new PreviewUnsubscribe(plainTokens, optional, store, ANA).execute({ token })).standing).toBe(
      'switched_off',
    );
  });
  ...
  it('calls an unsigned link unusable, and names no category', async () => {
    expect(
      await new PreviewUnsubscribe(plainTokens, optional, new FakeNotificationPreferenceStore(), ANA).execute({
        token: 'forged',
      }),
