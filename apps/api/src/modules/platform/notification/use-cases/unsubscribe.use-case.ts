 ...
 *
 * **A link that may switch off nothing is refused and writes nothing** — `followLink`'s answer, the one S-38's read
 * gives. **One already switched off succeeds**, since a second press, or a mail client posting after the person
...
export class Unsubscribe {
  ...
  async execute(command: UnsubscribeTokenQuery): Promise<UnsubscribePreview> {
    const link = await followLink({ ...command, tokens: this.tokens, categories: this.categories, recipients: this.recipients });
    if (link === null) throw new UnsubscribeLinkUnusableError();
    ...
    return { standing: UNSUBSCRIBE_STANDING.SWITCHED_OFF, categoryKey: subject.categoryKey, recipient: link.recipient };
  }
}
