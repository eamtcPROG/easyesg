 ...
 *
 * **In `realm/api/`, a kind of its own** (task 135). A transport, not a query definition — nothing
 * here imports TanStack Query — and not pure, so neither `queries/` nor `tools/` describes it;
 * `apps/web` keeps its seam at `server/api/` for the same reason. Every `queries/` module, the
 * realm's and each feature's, calls through this one client.
 */
