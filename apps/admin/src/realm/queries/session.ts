 ...
 *
 * **`realm/` is a leaf, and it is not a feature.** `admin-realm-is-a-leaf` forbids it importing
 * `features/**`: the session, the API client and A-01's screen are what every feature depends on,
 * so a reference back would make whatever it reached a dependency of both bounded contexts at once.
 * A-01 lives here rather than in `features/` because it serves PA and BO and belongs to neither.
 */
