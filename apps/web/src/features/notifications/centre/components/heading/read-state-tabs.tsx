...
import { CENTRE_SHOW, centreViewQuery, type CentreView } from '../../tools/centre-view';
 ...
 * and the one already shown says so with `aria-current`. Choosing either starts again at its first page — staying
 * on page 3 of a list that just became one page long would show nothing and read as *nothing matches*.
 *
...
export async function ReadStateTabs({ view, unread }: { readonly view: CentreView; readonly unread: number }) {
  const t = await getTranslations(`${CENTRE_MESSAGES}.show`);
  const tabs = [
    { show: CENTRE_SHOW.UNREAD, label: t('unread', { count: unread }) },
    { show: CENTRE_SHOW.ALL, label: t('all') },
  ];
      ...
      {tabs.map((tab) => (
        <li key={tab.show}>
          <Link
            ...
            href={withQuery(ROUTES.NOTIFICATIONS, centreViewQuery({ show: tab.show, page: 1 }))}
