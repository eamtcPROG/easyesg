...
import { CENTRE_ARM, centreArm } from '../../tools/centre-arm';
 ...
 * cannot be made until the view is parsed. **The empty state is the Index archetype's choice** (`centre-arm.ts`), so
...
export async function CentreSection({
  ...
  if (read.status === CENTRE_READ.FORBIDDEN) {
    ...
    const arm = centreArm(read.page);
    body =
      arm === CENTRE_ARM.LIST ? (
        <CentreList page={read.page} view={view} />
      ) : arm === CENTRE_ARM.FIRST_USE ? (
        <CentreFirstUse />
      ) : (
        <CentreNothingUnread />
      );
